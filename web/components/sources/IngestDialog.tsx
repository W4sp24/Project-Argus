...
export default function IngestDialog({
  ...
  useEffect(() => {
    if (!target && options.length) setTarget(options[0]);
  }, [options, target]);
  ...
  const wantsSummary = prompt.trim().length > 0;
  ...
  async function run() {
    ...
    try {
      const jobId = await startIngestJob(
        ...
        { target, summaryPrompt: prompt.trim(), replace },
      );
      onStarted(jobId);
      onClose();
    } catch (caught) {
    ...
    <>
      ...
      >
        ...
        <div className="mt-4 flex flex-col gap-4">
          ...
          <Field
            label="Summarize each file (optional)"
            hint="Argus writes the summary as a note beside the file, and indexes it too."
          >
            {(props) => (
              <textarea
                {...props}
                ...
                placeholder="e.g. list the key definitions and where they're introduced"
                rows={2}
        ...
        <p className="mt-3 font-mono text-meta text-ink-faint">
          {!wantsSummary || isLocal ? (
            <>Indexed on this machine — nothing leaves it.</>
          ) : (
            <>
              Indexed on this machine. Summarizing sends each file&apos;s text to{" "}
