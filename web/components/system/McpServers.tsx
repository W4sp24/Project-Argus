          ...
          return (
            <div key={server.name} className="border border-line p-3">
              <div className="flex items-center gap-2">
                ...
                <p className="min-w-0 flex-1 truncate font-mono text-label text-ink">
                  {server.name} <span className="text-ink-faint">:{server.port}</span>
                </p>
                {wired ? (
                  <span className="shrink-0 border border-ok px-1.5 py-0.5 font-mono text-micro uppercase tracking-[0.1em] text-ok">
                    WIRED
                  </span>
                ) : (
                  <button
                    ...
                    className="shrink-0 border border-line px-1.5 py-0.5 font-mono text-micro uppercase tracking-[0.1em] text-ink-faint transition-colors hover:border-lineHi hover:text-ink"
                  >
                    CONNECT
                  </button>
                )}
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {server.tools.map((tool) => (
                  <span key={tool} className="border border-line px-1.5 py-0.5 font-mono text-meta text-ink-faint">
                    {tool}
                  </span>
                ))}
              </div>
              <p className="mt-2 text-label leading-relaxed text-ink-muted">{server.detail}</p>
            </div>
          );
        })}
      </div>
