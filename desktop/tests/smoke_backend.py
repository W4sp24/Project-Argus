...
def _launch(target: Path) -> subprocess.Popen:
    cmd = (
    ...
    return subprocess.Popen(
        ...
        text=True,
        cwd=target.parent.parent.parent if target.suffix == ".py" else target.parent,
    )
...
def _watchdog_dies_with_parent(target: Path) -> bool:
    ...
    cmd = [sys.executable, str(target)] if target.suffix == ".py" else [str(target)]
    child = subprocess.Popen(
        [*cmd, "--parent-pid", str(dummy.pid)],
        stdin=subprocess.DEVNULL,
        stdout=subprocess.PIPE,
